...
Both return an `NPScheduleWork` — a pair of `resume` / `cancel` blocks. Throttle runs the callback
at most once per window (`cancel` has no effect, since the work already ran); debounce delays the
callback until the calls stop, so `cancel` can still call it off.

```Objective-C
NPScheduleWork throttled = NPDispatchScheduleThrottle(0.5, queue, ^{ /* task */ });
throttled.resume();

NPScheduleWork debounced = NPDispatchScheduleDeboundce(0.5, 0, queue, ^{ /* task */ });
debounced.resume();
debounced.cancel();
```
